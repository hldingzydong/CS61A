...
def reverse_recursive(lst):
    ...
    if len(lst) == 1:
        return lst
    res_lst = [lst[len(lst) - 1]]
    res_lst.extend(reverse_recursive(lst[:len(lst)-1]))
    return res_lst
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat_delta = get_lat(city_a) - get_lat(city_b)
    lon_delta = get_lon(city_a) - get_lon(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    mid_city = make_city('city_mid', lat, lon)
    if distance(city_a, mid_city) > distance(city_b, mid_city):
...
def add_chars(w1, w2):
    ...
    def add_chars_index(i1, i2):
        ...
        if w1[i1] == w2[i2]:
            return add_chars_index(i1+1, i2+1)
        else:
            return w2[i2] + add_chars_index(i1, i2+1)
    return add_chars_index(0, 0)
