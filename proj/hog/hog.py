...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while curr_roll < num_rolls:
        ...
        curr_res.append(dice())
    res = 0
    curr_roll = 0
    while curr_roll < num_rolls:
        if curr_res[curr_roll] == 1:
            return 1
        else:
            res = res + curr_res[curr_roll]
        curr_roll = curr_roll + 1
    return res
    # END PROBLEM 1
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_one_pos = player_score % 10
    opponent_one_pos = opponent_score % 10
    opponent_ten_pos = opponent_score // 10 % 10
    if abs(player_one_pos - opponent_one_pos) == opponent_ten_pos:
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while 1:
        ...
        plus_score = take_turn(num_rolls, score_list[the_other], dice)
        score_list[who] = score_list[who] + plus_score
        if feral_hogs:
            if abs(num_rolls - previous_score[who]) == 2:
                score_list[who] = score_list[who] + 3
        previous_score[who] = plus_score

        if is_swap(score_list[who], score_list[the_other]):
            tmp = score_list[who]
